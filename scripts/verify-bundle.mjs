...
if (manifest.builtWith?.types !== '0.8.7') {
    throw new Error(`Expected extension types 0.8.7, found ${manifest.builtWith?.types ?? 'unknown'}`)
}
...
const legacyClientName = ['paper', 'back'].join('')
const legacyDisplayName = legacyClientName[0].toUpperCase() + legacyClientName.slice(1)
if (homepage.includes(`${legacyClientName}://addRepo?`) || homepage.includes(`Add to ${legacyDisplayName}`)) {
    throw new Error('Homepage still contains a legacy install link')
}
...
console.log(`Verified extension repository with ${actualIDs.length} readable sources.`)
